...
export function SiteHeader() {
  return (
    <header className="relative sticky top-0 z-40 border-b border-white/60 bg-white/80 backdrop-blur-xl">
      <div className="grid min-h-20 w-full grid-cols-[1fr_auto] items-center gap-4 px-4 sm:px-6 md:grid-cols-[minmax(0,1fr)_auto_auto] lg:px-8">
        ...
        <nav className="hidden items-center justify-self-end gap-5 md:flex">
          {navLinks.map((link) => (
            <SiteNavLink
              key={link.href}
              href={link.href}
