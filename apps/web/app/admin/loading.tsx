...
export default function AdminLoading() {
  return (
    <Container className="space-y-10 py-10" role="status" aria-label="Loading source operations">
      ...
      {[0, 1, 2].map((section) => <section key={section} className="space-y-4"><Skeleton className="h-7 w-48" /><div className="rounded-xl border border-slate-200 bg-white p-5 dark:bg-zinc-900"><Skeleton className="h-10 w-full" /><Skeleton className="mt-4 h-16 w-full" /></div></section>)}
    </Container>
  );
}
