    ...
    <Container className="space-y-10 py-12">
      ...
      <section className="space-y-4">
        ...
        <Card className="rounded-xl shadow-none">
          <CardContent className="flex flex-col gap-5 md:flex-row md:items-end md:justify-between">
            ...
            <div className="border-l-2 border-slate-200 px-4 py-1 text-sm dark:border-slate-700">
      ...
      <section className="space-y-4">
        ...
        <div className="grid gap-4">
          {data.runs.map((run) => (
            <Card key={run.id} className="rounded-xl shadow-none">
              <CardContent className="space-y-2">
      ...
      <section className="space-y-4">
        ...
        {data.potentialDuplicates.length === 0 ? (
          <Card className="rounded-xl shadow-none"><CardContent>No duplicate clusters detected in the recent sample.</CardContent></Card>
        ) : (
          data.potentialDuplicates.map((cluster) => (
            <Card key={cluster[0].dedupeFingerprint} className="rounded-xl shadow-none">
              <CardContent className="space-y-4">
                {cluster.map((posting) => (
                  <div key={posting.id} className="border-l-2 border-slate-200 px-4 py-2 text-sm text-slate-600 dark:border-slate-700">
      ...
      <section className="space-y-4">
        ...
        <div className="grid gap-4 xl:grid-cols-2">
          {data.companies.map((company) => (
            <Card key={company.id} className="rounded-xl shadow-none">
      ...
      <section className="space-y-4">
        ...
        <div className="grid gap-4">
          {data.sources.map((source) => (
            <Card key={source.id} className="rounded-xl shadow-none">
