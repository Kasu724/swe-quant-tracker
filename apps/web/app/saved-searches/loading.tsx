...
export default function SavedSearchesLoading() {
  return (
    <Container className="space-y-10 py-10" role="status" aria-label="Loading saved searches">
      ...
      {["searches", "favorites"].map((section) => <section key={section} className="space-y-4"><Skeleton className="h-7 w-40" /><div className="space-y-3"><div className="rounded-xl border border-slate-200 bg-white p-5 dark:bg-slate-900"><Skeleton className="h-5 w-48" /><Skeleton className="mt-3 h-20 w-full" /></div><div className="rounded-xl border border-slate-200 bg-white p-5 dark:bg-slate-900"><Skeleton className="h-5 w-64" /><Skeleton className="mt-3 h-4 w-44" /></div></div></section>)}
    </Container>
  );
}
