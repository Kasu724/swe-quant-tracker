...
To announce newly discovered internships in a selected Discord server:
...
For one-off/serverless scheduling, run `pnpm worker:discord`.
...
Leave it empty unless using the legacy headless fallback; the Settings destination can be paused
without disabling ingestion. Never commit a real webhook URL.
